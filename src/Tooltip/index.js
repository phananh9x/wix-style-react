...
class Tooltip extends Component {
  static propTypes = {
    upgrade: allValidators(PropTypes.bool, (props, propName) => {
      if (!props[propName]) {
        deprecationLog(
          `Using "Tooltip" with current API is deprecated. In order to upgrade to the new Tooltip api just use "<Tooltip upgrade/>" and follow "5.1 Button" changed api docs. IMPORTANT! - After upgrading, when you import the react/enzyme "tooltipTestkitFactory", you will get an async testkit (all methods are async).`,
        );
      }
    }),
  };
